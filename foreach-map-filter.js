...
function showFirstAndLast(arr) {
    const newArr = [];
    arr.forEach(function (val) {
        const newStr = val.slice(0, 1) + val.slice(-1);
        newArr.push(newStr);
    });
    return newArr;
}
...
function myFind(arr, searchValue) {
    return arr.includes(searchValue) ? searchValue : undefined;
}
